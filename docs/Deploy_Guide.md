...
> **Canonical location:** This file lives in **`CID-docs/`** (outside any segment backend repo). Shared across **all segments** and future **RSS** canonical intake. Do not treat a copy inside `pdf-backend` (or another segment repo) as authoritative — edit here and use [README](./README.md) as the index.
